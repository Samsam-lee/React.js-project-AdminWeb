...
const AddressHashtag = (props) => {
  ...
  const handleDelete = async () => {
    await axios.delete(
      `http://13.125.69.16/admin/ytbCrawlingTb/video/delete/${props.errVideo.ytbChannel}/${props.errVideo.video[videoIndex]._id}`
    );
  };
  ...
  const fetchPlatformData = async () => {
    await axios.post(
        `http://13.125.69.16/admin/ytbCrawlingTb/address/search/${address}`
      ).then((res) => {
      ...
      {props.map ? (
        ...
        <>
          ...
          <Button width="700px" height="515px">
            {props.errVideo.video[0].more.map((v) => (
              <HashBox> {v} </HashBox>
            ))}
          </Button>
          <FlexDiv>
