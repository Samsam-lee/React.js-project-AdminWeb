...
const AddressHashtag = (props) => {
    ...
    const handleDelete = async () => {
        await axios.delete(`http://13.125.69.16/admin/ytbCrawlingTb/video/delete/${props.youtuber}/${props.addrHashtag.video[0]._id}`)
    ...
    const fetchPlatformData = async () => {
        await axios.put(`http://13.125.69.16/admin/ytbCrawlingTb/address/search/result/${address}`).then(res => {
    ...
    return (
        <Button width='750px' height='785px' position='relative'>
            ...
            <DcButton right='180px' onClick={handleReSearch}> 재검색 </DcButton>
            ...
            <><FlexDiv fontSize='22px' margin='25px'>주소 조합 후 검색</FlexDiv>
                ...
                <FlexDiv>
                    <DcButton right='170px' bottom='20px' onClick={handleDelete}> 삭제 </DcButton>
                    <DcButton right='30px' bottom='20px' onClick={handleSearch}> 검색 </DcButton>
            </FlexDiv></>}
        </Button>
    )
}
